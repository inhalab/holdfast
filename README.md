...
**p95를 이 표에 싣지 않는다. 전략을 구분할 수 있는 값이 아니기 때문이다.** 다섯 전략이
...
**더 정확히는 "전략 간 차이가 없다"가 아니라 "이 지표로는 볼 수 없다"이다.**
고경합 회당 요청 59,000건 중 확정에 성공해 락 경로를 끝까지 지나는 것은
**20건(0.034%)**뿐이다. 나머지는 이미 팔린 좌석을 만나 즉시 돌아서므로 전략이
갈리는 구간을 지나지 않는다 — 위 p95는 사실상 **409 거절의 p95**다.
**차이가 없다고 말할 근거는 없고, 이 표로는 볼 수 없다고 말할 근거만 있다.**
...
**답하지 못한 질문도 그대로 남겼다** — 임계 구역이 더 길어지면 전략이 갈리는가는
이 하네스로 잴 수 없었다([M3 결론](docs/results/m3-conclusion.md) 3.5절).
...
infra/       Terraform, 배포
load-test/   k6 시나리오
tools/       저장소 도구 — 마크다운 렌더러(시연 대본용), 발표 자료 생성기
