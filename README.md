...
| DB 유니크 제약 | **0** ✓ | 0 | 35ms | 488 TPS | 99.97% | **11** ※ |
| Redis 분산락 | | | | | | |
...
**DB 제약만으로도 초과 확정 0이다.** 앱 레벨 락을 전혀 쓰지 않는 전략인데
9회 전부 초과 확정·초과 홀드가 0이다. **앱 락은 경합이 드러나는 방식을 바꿀 뿐
정합성에는 기여하지 않았다** — 같은 사건이 비관적 락에서는 행 락 대기로,
낙관적 락에서는 재시도로, 여기서는 제약 위반으로 나타난다.

**※ 제약 위반 11건은 이 전략에서 정상 동작이다.** 같은 좌석을 동시에 노린
요청이 그만큼 있었다는 뜻이고, **앱 락을 쓰는 전략에서는 락이 흡수해 0으로
보이던 숫자**다. 나머지 전략에서 이 값이 0이 아니면 반대로 앱 락이 샜다는
신호다([4.4](docs/concurrency-spec.md)).

**낙관적 락도 초과 확정 0이다.** 세 경합도 전부 0이며, 재시도 102회·소진
34건으로 이 전략 고유 비용은 드러난다(비관적 락은 락 포기 0). 전략마다 정합성을
지키는 방식이 다를 뿐 결과는 같다.

**베이스라인의 초과 확정은 경합의 함수다.** 고경합 4석·극단 1석에서 나오고
저경합(1000석/100VU)에서는 0이다 — 같은 좌석을 동시에 노릴 일이 없으면 깨질
것도 없다. 저경합에서 처음 관측된 값은 측정 절차의 결함이었고 폐기했다
([폐기 기록](docs/results/discarded-measurements.md)).
