...
--
-- **좌석번호 체계가 /demo/race의 시드 버튼과 겹친다.** 이 파일은 A구역에
-- A-1~A-12를 id 100~111로 넣고, 그 버튼은 같은 구역에 A-1~A-n을 id 1~n으로
-- 넣는다. U-4가 (zone_id, seat_no)에 걸려 있으므로 **id는 안 겹치는데
-- 좌석번호가 겹친다** — 그 버튼의 멱등 가드가 id만 보다가 500을 냈다(#154).
--
-- **지금은 그 버튼이 A구역을 자기 것으로 다시 만든다.** 이 시드를 돌린 뒤에
-- 눌러도 되고, 눌러도 이 파일이 만든 B구역은 남는다(재고가 없어 화면에는
-- 안 보인다). **가드를 id 축으로 되돌리지 마라** — 그러면 같은 500이 돌아온다.
--
