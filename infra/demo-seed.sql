...
-- **id를 명시해 넣었으므로 시퀀스를 맞춘다.**
--
-- 스키마가 GENERATED BY DEFAULT AS IDENTITY라 명시적 INSERT가 통과하지만,
-- 그때 시퀀스는 올라가지 않는다. 맞춰 두지 않으면 관리자 화면(#101)이 id 없이
-- INSERT할 때 시퀀스가 1을 주고 이미 있는 행과 충돌한다 — 실제로 프로그램
-- 등록이 duplicate key로 500이 났다.
SELECT setval(pg_get_serial_sequence('program', 'id'),
              COALESCE((SELECT MAX(id) FROM program), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('seat_layout', 'id'),
              COALESCE((SELECT MAX(id) FROM seat_layout), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('zone', 'id'),
              COALESCE((SELECT MAX(id) FROM zone), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('seat', 'id'),
              COALESCE((SELECT MAX(id) FROM seat), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('event_session', 'id'),
              COALESCE((SELECT MAX(id) FROM event_session), 0) + 1, false);
