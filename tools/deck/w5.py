...
s.header("05", "태그가 같은 날 두 번 거짓말했다",
         "태그는 «우리 것»을 가리는 기준이지 «지금 실제로 있는 것»을 가리는 기준이 아니다")
...
s.text(M, 536, CW, 36, "그래서 태그가 아니라 실물을 직접 센다", 27, INK, True)
s.text(M, 584, CW, 60,
       ["ECS 서비스 · 실행 태스크 · RDS · ALB 를 AWS 에 직접 조회해 하나라도 있으면 실패로 끝낸다.",
        "이 판정이 apply 를 중간에 죽여 state 밖에 남은 RDS 를 실제로 잡았다 — state 를 믿었다면 «destroy 성공»으로 끝났다."],
       18.75, MUTED, lh=1.5)
tail(s, "«태그가 남았나»가 아니라 «실제로 있나»다. 확인은 거짓 실패도 거짓 통과도 내면 안 된다.")
