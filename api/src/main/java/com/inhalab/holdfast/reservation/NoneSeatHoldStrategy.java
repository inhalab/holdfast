 ...
 * INSERT를 막지도 않는다.
 *
 * <p><b>그래서 이 전략에는 만료 홀드를 회수하는 경로가 아예 없다.</b> 나머지 넷은
 * 홀드 경로의 lazy 검증이 정리하고, 청소 스케줄러는 구현돼 있지 않다
 * (concurrency-spec 3절). 결함이 아니라 "아무 방어도 하지 않는다"는 베이스라인의
 * 정의이며, 지속 경합 시나리오에서 {@code none}의 좌석이 돌아오지 않는 이유이기도
 * 하다(7.2.2).
 *
