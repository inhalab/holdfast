...
### 0.1 어디서 도는가 — 개발 PC에서 돌고 청중은 터널로 붙는다

**AWS가 아니다.** 라이브 시연은 위 명령 그대로 **발표자 개발 PC의 Docker
Compose**에서 돌고, 청중은 **Cloudflare Tunnel**로 붙는다. 판정과 근거는
[`infra-decision.md`](infra-decision.md) 2.1에 있다.

| 누가 | 어디로 |
|---|---|
| **발표자** | `http://localhost:8080` — **전부. `/admin`도 여기서 연다** |
| **청중** | `https://demo.<도메인>` — 좌석맵 · `/demo/race` · `/my/reservations`만 |

**AWS는 화면 하나를 띄워 보이는 자리**이지 라이브가 아니다. **당일 AWS가 안 떠도
발표가 된다.**

#### 터널은 시연 직전에 열고 직후에 닫는다

**상시로 두지 않는다.** 터널이 열려 있는 동안 개발 PC가 인터넷에 나간다.

**터널을 연 직후, 그 주소로 `/admin/programs`가 404인지 확인한다.** `admin`은
시연에서도 **켜져 있고**(3절이 그것을 쓴다) 청중에게 안 보이게 하는 것은 터널
ingress뿐이다. **설정이 틀리면 조용히 열리는데, 발표자는 `localhost`로 보므로
정상으로 보인다.**

#### 리허설도 터널로 한다

**로컬 직접 접속과 터널 경유는 조건이 다르다.** 직접은 HTTP/1.1이라 브라우저가
한 번에 6개만 보내고(실측), 터널을 쓰면 그보다 많이 나간다. **리허설을
`localhost`로 하면 발표장에서 다른 숫자가 나온다.**

**다만 "같아진다"가 아니라 "차이가 줄어든다"이다** — 프로토콜은 같아져도 **집
네트워크와 발표장 네트워크가 다르다.** 숫자가 왜 다른지와 대본에 숫자를 어떻게
쓸지는 **#170이 다룬다.**

#### 안 되면 직접 접속으로 간다

터널이 안 열리거나 발표장 네트워크가 막으면 **발표자 화면의 `localhost:8080`으로
그대로 진행한다.** 청중 참여만 빠지고 나머지 절은 다 돈다. **동시 도착이 6으로
줄어 숫자는 달라지지만 발표는 된다.**

**이 fallback이 필요한 이유는 의존성이 줄지 않고 바뀌기 때문이다** — 지금 시연은
아무것에도 안 기대는데(`localhost`), 터널을 쓰면 **발표장 네트워크와 Cloudflare
엣지**에 건다.
