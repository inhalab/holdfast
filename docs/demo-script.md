...
### 0.1 어디서 도는가 — 개발 PC에서 `localhost`로 돈다

**AWS가 아니다.** 라이브 시연은 위 명령 그대로 **발표자 개발 PC의 Docker
Compose**에서 돌고, **주소는 `http://localhost:8080` 하나다.** 판정과 근거는
[`infra-decision.md`](infra-decision.md) 2.1에 있다.

| 누가 | 어디로 |
|---|---|
| **발표자** | `http://localhost:8080` — 전부. `/admin`도 여기서 연다 |
| **청중** | **붙지 않는다.** 화면을 본다 |

**AWS는 도메인이 붙은 실제 서비스로 띄워 한 번 보인다** — 배포 증거다. 라이브는
아니므로 **당일 AWS가 안 떠도 발표가 된다.**

#### 청중 참여와 터널은 회수됐다

**한때 이 대본은 청중이 Cloudflare Tunnel로 붙는다고 적었다.** 터널을 실제로 재서
뺐다 — 같은 조건에서 동시 도착이 **1차 19~20, 2차 86·80으로 4.5배 갈렸고**,
**무엇이 그것을 정하는지 모른다.** 처리량도 4배 느렸다. 표는 2.1에 있다.

**리허설에서 본 숫자가 발표장에서 안 나오면 시연이 자기가 무엇을 보였는지
말하지 못한다.** 그래서 통제되는 쪽을 골랐다.

#### 그래서 리허설이 곧 발표다

**조건이 같다.** 리허설도 발표도 `./holdfast demo`로 띄운 `localhost:8080`이고,
네트워크를 지나지 않으므로 **발표장 사정에 흔들리지 않는다.** 터널을 쓸 때 필요했던
직접 접속 fallback도, 터널을 연 뒤 `/admin`이 밖에서 404인지 확인하는 절차도
**함께 사라진다.**

#### 동시 도착은 6이다 — 그렇게 말한다

로컬 직접 접속이라 브라우저가 **호스트당 6개까지만** 동시에 보낸다. 실측에서
200발을 눌러도 동시 도착이 **6에서 40초 동안 움직이지 않았다** — 화면 코드에
상한이 없는데도 그렇다.

**"동시에 200개를 쐈습니다"라고 말하지 않는다.** 대신 이렇게 말한다.

> 브라우저는 호스트당 6개까지만 동시에 보냅니다. 그래서 좌석 하나에 몰아쳐
> **6중 경합**을 만듭니다. 진짜 동시성 숫자는 뒤에 나올 **k6 500 VU**에 있습니다.

**숨기는 것보다 낫다.** `scope-m4.md` 7절이 이 제약과 우회를 이미 적어 두었고,
전송 계층까지 짚었다는 신호가 된다. 문안은 **#170이 다룬다.**
