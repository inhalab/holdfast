...
3. **결제** → 예약 확인 화면으로 이동, QR 토큰이 나온다.
   **이 절에서는 `?userId=`를 건드리지 않는다.** 좌석맵은 연 주소의 userId로
   예약을 넣지만(`X-User-Id`), 결제 뒤 예약 확인으로 보내는 리다이렉트는 그 값을
   싣지 않아 **사용자 1로 열린다** — 2로 예약했으면 여기서 404다. 예약 확인은
   소유자만 볼 수 있고 그것이 의도된 동작이다(`scope-m4.md` 8절).
   **사용자를 바꿔 보이는 것은 1절의 몫이다**
...
3. 그 회차를 사용자 화면에서 연다 — 방금 만든 것이 바로 팔린다.
   **다만 여기서 열린 좌석맵으로는 동시성을 보일 수 없다.** 관리자 회차 목록의
   ...
   (`scope-m4.md` 8절 — 좌석맵에는 소유권 검사가 없어 링크가 그 값을 실을 이유가
   없다). 그런데 "같은 좌석을 여럿이 동시에 노린다"는 창마다 userId가 달라야
   성립한다(6절). 보여야 한다면 둘 중 하나다.

   - 주소창에서 `?userId=2`를 붙여 창을 하나 더 연다
