...
## 2.2 `event_session`의 시각 관계

다섯 시각이 서로 어떤 순서여야 하는지가 어디에도 없었다. 그래서 관리자 등록이
**시작보다 17일 앞선 종료**, **입장 종료보다 늦은 예약 오픈**을 조용히 저장했다.
아무도 예약할 수 없고 예약해도 입장할 수 없는 회차다.

### 각 시각이 무엇을 가르는가 — 코드에 있는 것부터

| 컬럼 | 무엇을 막는가 | 어디서 |
|---|---|---|
| `reserve_opens_at` | 이전이면 홀드가 409 `RESERVATION_NOT_OPEN` | `SeatHoldService` |
| `entry_opens_at`·`entry_closes_at` | 밖이면 검표가 `REJECTED_TIME` | `TicketService` |
| `status` | `OPEN`이 아니면 카탈로그에서 고를 수 없다 | `CatalogPageController#saleStateOf` |
| `starts_at`·`ends_at` | **판정에 쓰이지 않는다.** 회차 자체의 시간이며 화면 표시용이다 | — |

### 지켜야 할 네 가지

| # | 규칙 | 어기면 |
|---|---|---|
| T-1 | `starts_at < ends_at` | 회차가 끝난 뒤에 시작한다 |
| T-2 | `entry_opens_at < entry_closes_at` | `TicketService`의 두 비교가 동시에 참이라 **아무도 입장할 수 없다** |
| T-3 | `reserve_opens_at < entry_closes_at` | 예약이 열리는 시점에 입장이 이미 끝났다. 예약할 수 있게 되어도 쓸 수 없는 표다 |
| T-4 | `entry_opens_at < ends_at` | 회차가 끝난 뒤에 문이 열린다 |

**T-3이 `reserve_opens_at < starts_at`이 아닌 이유**는 회차가 시작한 뒤의 판매를
막을 근거가 없기 때문이다. 늦게 온 사람이 사서 들어가는 것은 정상이며, 그 표가
쓸모없어지는 경계는 시작이 아니라 **입장 종료**다.

**`entry_closes_at ≤ ends_at`은 넣지 않는다.** 지금 시드가 전부 그것을 어긴다 —
`infra/demo-seed.sql`이 종료 +3시간에 입장 종료 +6시간이다. 퇴장 여유를 두는 것은
정상 운영이고, 어겨서 깨지는 것이 없다.

### 상태와 데이터의 관계

| # | 규칙 | 근거 |
|---|---|---|
| S-1 | `status`는 `SCHEDULED`·`OPEN`·`CLOSED` 셋뿐 | 2절 정의. 지금은 자유 문자열이라 아무 값이나 저장된다 |
| S-2 | 예약이 하나라도 있으면 `SCHEDULED`로 되돌릴 수 없다 | `SCHEDULED`는 "아직 판 적 없다"는 뜻이다. 카탈로그에서는 `NOT_YET_OPEN`으로 사라지는데 발권된 티켓은 그대로 살아 있어(검표는 `status`를 보지 않는다) **화면과 티켓이 갈린다.** `CLOSED`로 가는 것은 접수 종료라 허용한다 |
| S-3 | 판매된 좌석이 있으면 `reserve_opens_at`을 **미래로** 옮길 수 없다 | 이미 판 사실과 모순된다. `saleStateOf`가 `NOT_YET_OPEN`을 띄워 **산 사람이 자기 회차를 목록에서 못 찾는다** |
| S-4 | `max_per_user`를 이미 그보다 많이 가진 사람이 있는 값으로 줄이는 것은 **허용하되 알린다** | CS-6은 홀드 시점에 `held_count + n ≤ max_per_user`를 볼 뿐이라(`concurrency-spec.md` 1.1) 기존 예약은 소급해서 무효가 되지 않는다. **막으면 한 번 넉넉히 잡은 상한을 영영 못 고친다.** 이미 판 표를 뺏는 것은 더 나쁘다 — 그래서 기존은 두고 신규만 막고, 몇 명이 초과 상태인지 화면에 띄운다 |
| S-5 | 입장 창 변경은 판매 뒤에도 허용한다 | 일정 변경은 정상 운영이고, 티켓이 회차를 참조하므로 새 창이 그대로 반영된다. 다만 영향받는 티켓 수를 알린다 |
| S-6 | **예약이 하나도 없는 회차만 지운다** | S-2를 연장한 것이다. 되돌리기가 "판 적 없다고 말하는 것"이라면 삭제는 **"판 기록을 없애는 것"**이라 더 강하다. 되돌리기를 막았으면서 삭제를 허용할 근거가 없다. 예약을 받은 회차를 닫는 수단은 `CLOSED`로 이미 있다 |

#### S-6이 S-2의 조건을 그대로 쓰지 않는 이유

**S-2는 살아 있는 예약만 센다**(`status NOT IN ('CANCELLED','EXPIRED')`).
그 조건으로 삭제를 판정하면 **만료된 예약만 남은 회차가 "지워도 된다"로 통과하고
곧바로 `fk_reservation_session`에 걸린다.**

두 규칙이 묻는 것이 다르다.

| | 묻는 것 |
|---|---|
| S-2 (되돌리기) | 지금 **유효한** 예약이 있는가 |
| S-6 (삭제) | 이 회차에 **무슨 일이든** 있었는가 |

그래서 S-6은 `status`를 가리지 않고 `reservation` 전체 행을 센다.

#### 함께 지우는 것은 둘뿐이다

`event_session`을 참조하는 FK는 넷인데 삭제 시점에 걸릴 행은 둘이다.

| 테이블 | 예약 없는 회차에 남아 있는가 |
|---|---|
| `seat_inventory` | **있다** — 회차 등록이 사전 생성한다 |
| `user_session_quota` | **있다** — 같이 사전 생성된다 |
| `seat_hold` | **없다** — 아래 근거 |
| `reservation` | **없다** — 있으면 삭제 자체를 거절한다 |

**`seat_hold`에 걸릴 행이 없는 근거**는 4절이다. `SeatHoldService`가 홀드를 넣는
**같은 트랜잭션에서** `reservation` 행을 만들고("예약은 홀드 시점에 생성된다"),
만료돼도 그 행을 지우지 않는다. **예약이 0이면 홀드도 0이다.**

#### 세는 것과 지우는 것 사이를 잠근다

그 사이에 홀드가 들어오면 FK 위반으로 500이 된다. 삭제하는 쪽이
`SELECT ... FOR UPDATE`로 회차 행을 먼저 잠근다 — 예약 INSERT가 부모 행에 거는
`KEY SHARE`와 충돌하므로 그 창이 닫힌다. **홀드 경로는 바뀌지 않는다.** 그쪽이
부모 행에 `KEY SHARE`를 거는 것은 FK가 원래 하던 일이다.

**`seat_layout_id`·`program_id`를 못 바꾸는 것은 이미 `updateSession`에 있다** —
배치도를 바꾸면 `seat_inventory`가 다른 배치도의 좌석을 가리킨다.

### 판정은 여전히 실행 시각으로 한다

이 규칙들은 **등록 시점의 입력을 막는 것**이지 판정을 대신하지 않는다.
`SeatHoldService`·`TicketService`는 지금처럼 `now()`와 비교한다. 등록 검증이
통과했다고 그 회차가 지금 예약 가능하다는 뜻은 아니다.
...
(`CONFIRMED→CANCELLED`)가 담당한다 — **위 U-2 문단의 "같은 사실을 두 곳에서 지키면
어긋난다"와 같은 원칙이다.** `payment` 행은 승인·거절·실패로 한 번 종결되면 그
자체로는 바뀌지 않는 이력이고, 예약이 나중에 취소돼도 "그 결제 시도가 승인됐었다"는
사실은 변하지 않는다.

> 이 문단은 한때 그 원칙의 출처를 **3.1절**로 적었다. 3.1은 "U-2를 `none`에서만
> 제외하는 이유"이고 그 원칙이 있는 곳이 아니다. 원칙은 이 4절 안, U-2의 조건에
> `CONFIRMED`를 넣지 않는 문단에 있다.

**이 판단을 이슈 #106에서 다시 검토했고, 유지한다.** #106("예약 취소 후 환불 상태
전이")이 요구한 것이 **정확히 이 판단을 뒤집는 일**이었다. 그 이슈는
`rfp-scope.md` 4.2의 "취소는 구현됐으나 환불 상태가 없다. 실제 환불은 불가하나
상태 전이는 가능"에서 나왔다. **그 서술이 이 문단보다
나중이다**(2026-09-04 `#111` 대 2026-09-01 `#33`). 그러나 `#111`은 요구사항 원문에서
후보를 뽑는 작업이었고 `erd.md`를 건드리지 않았다 — **RFP의 SFR-004에 "환불"이 있다는
것만 보고 적힌 줄이지, 이 판단을 읽고 뒤집은 것이 아니다.**

**뒤집지 않는 이유가 넷이다.**

**첫째, 정보가 늘지 않는다.** `payment.status = REFUNDED`는
`reservation.status = 'CANCELLED'`와 그 예약의 `payment.status = 'APPROVED'`의
**순수 함수**다. 부분취소는 범위 밖이라 금액이 갈리지 않고(`rfp-scope.md` 3장
SFR-004), 실제 PG가 없어 환불이 실패할 수 없으며, 환불 시각은 취소 시각과 같아
`reservation.cancelled_at`이 이미 들고 있다. **새 컬럼이 담을 새 사실이 없다.**

**둘째, 없는 절차를 있다고 말하게 된다.** 돈이 움직이지 않는데 화면이 "환불
완료"라고 적으면 그것은 감추는 것보다 나쁘다 — **없는 사실을 주장한다.**
`design-spec.md` 5.6이 접수종료를 감추지 않고 상태를 적기로 한 것과 방향이 반대다.

**셋째, `TicketStatus.VOID`를 두지 않은 판단과 같은 모양이다**
(`state-transitions.md` 4절). 그쪽도 설계 단계에 잡았다가 구현에서 뺐고, 이유가
둘이었다 — 상태 값 없이도 조회 시점에 판정할 수 있고, 취소 트랜잭션에 쓰기가
하나 늘면 **CS-4의 임계 구역이 길어진다**. 환불도 똑같다. "이 예약이 환불
대상인가"는 취소 시점에 저장할 것이 아니라 **읽는 시점에 예약 상태와 결제 상태를
함께 보면 나온다.**

**넷째, 두 곳에 두지 않는 것이 SFR-004가 말하는 "일관"을 지키는 가장 강한
방법이다.** 검수 기준의 "취소·환불 결과가 결제·예약 시스템 간 일관"은 **두 시스템이
분리돼 있을 때**의 요구다. 이 프로젝트에서 Mock PG는 앱 안의 클래스이고 `payment`와
`reservation`은 같은 트랜잭션 안에 있어, 일관성은 이미 트랜잭션이 보장한다.
**상태를 하나 더 두는 것은 어긋날 수 있는 자리를 새로 만드는 것이다.**

> **`#200`이 그 임계 구역을 처음으로 재 봤다.** 위 셋째가 *"쓰기가 하나 늘면
> CS-4의 임계 구역이 길어진다"*고 적었는데, **그 구역이 지금 얼마나 되는지는
> 아무도 잰 적이 없었다** — 검증 쿼리도 부하 경로도 없었다. `concurrency-spec.md`
> 7.2.4가 그것을 밟는다.
>
> **이 판단은 그대로다.** `#200`은 재는 것이지 쓰는 것이 아니며, 취소 트랜잭션이
> 건드리는 넷(`seat_inventory` · `seat_hold` · `reservation` · `user_session_quota`)은
> 변하지 않았다.

**그래서 `PaymentStatus`는 다섯 값 그대로다.** 이 판단은
`MinimumScopeFlowTest`가 행동으로 고정한다 — **취소해도 그 예약의 `payment` 행은
`APPROVED` 그대로 남는다.** 누군가 취소 경로에 결제 상태 전이를 넣으면 그 테스트가
깨지고, 깨진 사람이 이 문단으로 온다.
...
**이 표는 화면에서도 읽힌다.** 좌석 조회가 만료된 홀드를 `HELD_EXPIRED`로
드러내는데(#157), **그 좌석을 실제로 인수할 수 있는지는 아래 표가 정한다.** 네
전략은 인수하고 `none`은 하지 않으므로, `none`에서는 `HELD_EXPIRED` 좌석을 눌러도
409로 거절된다. **화면이 거짓말하는 것이 아니라 `none`이 그렇게 정의된 것이다** —
조회는 "홀드가 만료됐다"는 사실만 말하고 인수 가부는 말하지 않는다.
...
`design-spec.md` 4.2절의 "여유가 되면" 항목(대기열, 노쇼 처리)도 현재 ERD에
포함하지 않는다. 착수가 결정되면 그 시점에 이 문서를 갱신한다.

**비회원 조회는 착수했고(#103, REQ-14) 새 테이블이 필요 없었다.** 조회 화면 둘 다
`reservation`·`reservation_seat`·`ticket`을 읽기만 한다. **확인 정보용 컬럼을
더하지 않은 이유**는 `requirements.md` 3절에 있다 — 이름·연락처를 넣으려면
홀드·확정 경로가 그 값을 받아야 하고 그것이 측정 경로다.
