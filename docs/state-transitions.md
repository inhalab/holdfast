...
| `CONFIRMED → CANCELLED` | API 호출 — `POST /api/reservations/{id}/cancel` | `seat_inventory`(`SOLD→AVAILABLE`), `seat_hold`(`CONFIRMED→RELEASED`), **`user_session_quota`(감소)** | **`seat_hold` 해제의 `rowsAffected`가 게이트다** — 0이면 다른 취소가 이미 끝낸 것이라 좌석도 카운터도 건드리지 않는다(`#209`). 재호출 시 200과 기존 결과(`api-spec.md` 6.1절 멱등). **동시 호출은 `concurrency-spec.md` 7.2.4와 `CancelConcurrencyTest`가 본다** |
...
> **처음에는 안 섰다.** 상태 검사를 할당량 행을 잠그기 **전에** 읽은 스냅샷
> 위에서 하고 있어, 겹친 취소 둘이 그 검사를 **둘 다 통과**했다 — 보유량이
> 겹친 수만큼 깎였다(`#209`). **`seat_hold` 해제의 `rowsAffected`를 게이트로
> 세워 고쳤고**, 그것은 `erd.md` 4.1이 만료 정리에 쓴 관용구와 같다.
...
**`CANCELLED`는 이 상태 기계에 없다. 환불 상태도 두지 않는다.** `payment` 행은
`APPROVED`/`DECLINED`/`FAILED`로 한 번 종결되면 그 자체로는 바뀌지 않는 이력이다.
예약이 나중에 취소돼도 결제 취소 전용 상태가 필요하지 않은 이유와 판단 근거는
`erd.md` 4절에 있다.

**이슈 #106에서 다시 검토했고 유지했다.** 그 이슈는 환불 상태를 더하자는 것이었고,
`erd.md` 4절이 근거 넷으로 유지를 판정했다 — 요지는 **환불 상태가 예약 상태와 결제
상태의 순수 함수라 새 사실을 담지 않는다**는 것이다.

**4절의 `VOID` 판단과 나란히 읽는다.** 둘 다 설계 단계에 잡았다가 뺐고, 뺀 자리를
**조회 시점의 판정**이 메운다. 검표는 `TicketService#scan`이 예약 상태를 함께 읽어
막고, 환불 대상 여부는 취소된 예약에 `APPROVED` 결제가 있는지로 그 자리에서 나온다.
**둘 다 취소 트랜잭션에 쓰기를 더하지 않는다** — CS-4의 임계 구역을 늘리지 않는
것이 같은 이유의 같은 결과다.
