...
### 1.1 이 표는 어디서 왔나 — 40개를 줄인 것이 아니다

**이 절이 없어서 "제안요청서 40개를 14개로 줄였다"로 읽혔다.** 2주차 계획서
발표에서 설명한 구성인데 저장소에 자리가 없었다(#135).

**뺄셈이 아니다.** 제안요청서의 40개는 **분류 체계**이고 프로젝트 관리(PMR)·
보안(SER)·품질(QUR)·하자보수처럼 **학부 2인이 손댈 수 있는 것이 아닌 항목**을
포함한다. 무엇을 왜 제외했는지는 `rfp-scope.md` 4장에 있다. 이 표는 **남은 것을
어디서 가져왔고 무엇을 더했는가**를 적는다.

#### 출처별 구성

**개수와 목록을 함께 둔다.** 한쪽만 두면 어긋나도 드러나지 않는다 — 테스트 내역
합계가 88로 어긋났던 것이 그 형태였다(#127).

| 출처 | 개수 | REQ |
|---|---|---|
| **국립중앙과학관** RFP | **9** | REQ-01 · 02 · 03 · 04 · 05 · 06 · 10 · 13 · 14 |
| **궁능유적본부** RFP | **2** | REQ-07(SFR-03) · REQ-08(SFR-05) |
| **자체 확장** | **3** | REQ-09 · 11 · 12 |
| **합계** | **14** | REQ-01 ~ REQ-14 |
...
| REQ | 무엇 | 왜 원본에 없나 |
|---|---|---|
| **REQ-09** 좌석 단위 점유 | 정원이 아니라 **좌석 하나하나**를 잡는다 | **원본은 정원 기반이다.** 정원 차감은 카운터 하나라 **락 전략이 갈릴 자리가 없다** — 어느 전략이든 같은 행 하나를 두고 다투므로 다섯을 비교할 대상이 사라진다. **이 확장이 없으면 이 프로젝트가 성립하지 않는다** |
| **REQ-11** CS-6 | 1인 최대 매수가 **동시 요청에서도** 지켜지는가 | 설계 중 드러난 동시성 요구사항 — 3절 |
| **REQ-12** 만료 홀드 정리 | 정리와 INSERT가 갈리면 **측정이 오염된다** | 설계 중 드러난 측정 무결성 요구사항 — 3절 |

**REQ-11·12를 REQ-03·01 아래에 끼우지 않고 따로 세운 기준은 3절에 있다.**
요지는 하나다 — 끼워 넣으면 그 항목이 요구하는 **별도의 검증이 표에서 보이지
않게 된다.**
...
>
> **`#200`이 그 조건을 건드리지 않았다 — 재는 것이지 쓰는 것이 아니다.** 취소
> 트랜잭션이 건드리는 것은 그대로 넷이고, **REQ를 새로 세우지 않았다.**
> 바뀐 것은 **검증 방법 열**이다. 위 문단이 근거로 든 멱등성은 **순차 재호출을
> 전제**했는데, 동시 호출에서도 서는지를 이제 `CancelConcurrencyTest`와
> 7.2.4의 부하 시나리오가 본다. **2절이 경계한 「구현했지만 검증 수단이 없음」에서
> CS-4가 빠진 것이고, REQ가 하나 늘어난 것이 아니다.**
