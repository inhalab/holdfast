...
### 검증이 있는데 못 잡은 사례가 하나 나왔다 (#143)

**이 절은 "미구현"과 "구현했지만 검증 수단이 없음"을 갈랐고 뒤엣것이 더
위험하다고 적었다. #143은 그보다 나쁜 셋째 자리다 — 검증이 있고 통과하는데
못 잡는다.**

없는 전략 이름이 `HOLDFAST_STRATEGY`로 들어가 **컨테이너가 기동하지
못했는데 테스트 123건이 전부 통과했다.** 슬라이스라서가 아니다 — 전략
테스트는 전부 `@SpringBootTest`로 전체 컨텍스트를 띄운다.

| 못 잡은 이유 | |
|---|---|
| **잘못된 값이라는 입력이 없다** | 모든 테스트가 `properties = "holdfast.strategy=..."`로 유효한 값을 직접 준다 |
| **실행 경로를 건너뛴다** | 그 `properties`가 `application.yml`의 `${HOLDFAST_STRATEGY:none}` 자리표시자를 지나지 않는다 |

**교훈은 "테스트를 더 짜라"가 아니라 "무엇이 값을 주는가"다.** 검증이 실제
실행기와 다른 경로로 값을 받으면, 그 검증은 실행기가 만드는 오류를 구조적으로
볼 수 없다. `StrategyArgumentTest`가 실행기와 애너테이션의 목록을 묶어 그
구멍 하나를 막았고, **이미지가 실제로 뜨는지를 묻는 검증은 #144로 남겼다.**
