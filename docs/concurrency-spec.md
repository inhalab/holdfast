...
| 프론트엔드 | React+Vite vs Thymeleaf | 미결정 |
...
프론트엔드는 `design-spec` 5.3이 좌석맵 최소 구현을 지시하고 있으므로, 담당자가 익숙한 쪽을 택한다. 격자 수준 UI에서는 SPA의 이점이 크지 않고, 서버 렌더링으로 가면 언어가 하나로 줄고 빌드 파이프라인이 없어진다.
