...
export const PROFILES = {
  ...
  // 7.2.3 할당량 경합(CS-6). **userPool이 VU보다 훨씬 작은 유일한 프로파일이다.**
  //
  // 위 셋은 "동시에 도는 반복이 전부 다른 사용자"가 되게 풀을 VU 이상으로 두는데,
  // 그러면 CS-6이 구조적으로 안 밟힌다 — 같은 사용자의 요청 둘이 서로 다른
  // 좌석을 동시에 노려야 그 구역이 열린다(1.1). 여기서는 일부러 뒤집는다.
  //
  // 좌석을 1000으로 두는 것도 그 때문이다. 좌석이 모자라면 좌석 행에서 먼저
  // 직렬화되어 할당량 구역까지 가지 못한다.
  //
  // **scripts/seed.sh의 quota 항목과 반드시 같은 값이어야 한다.**
  quota: { seats: 1000, vus: 500, userPool: 20, purpose: 'CS-6 정합성' },

  // 7.2.4 취소·재선점(CS-4). **확정까지 가는 유일한 프로파일이다.**
  //
  // 위 넷은 홀드에서 끝나거나(7.2.2·7.2.3) 확정해도 되돌리지 않는다. CS-4는
  // 좌석이 SOLD 가 된 뒤 돌아오는 구간이라, 확정과 취소가 둘 다 있어야 밟힌다.
  //
  // **좌석 수를 파일럿 3회로 확정했다**(7.2.4). 처음에는 "적을수록 경합이 세다"로
  // 잡았는데 파일럿이 뒤집었다 — 좌석을 줄여도 **재판매 간격의 하한이 거의 안
  // 움직인다**(10석 17ms · 50석 13ms). 돌아온 좌석을 다시 팔려면 홀드와 확정
  // 두 왕복이 필요해서다. 그래서 좌석 수의 유효한 효과는 노출량뿐이고,
  // 노출량은 좌석이 많을수록 크다(10석 1,354쌍 · 200석 11,671쌍).
  //
  // **다만 노출량을 늘려도 재선점 창이 열리지는 않는다.** 그 한계는 7.2.4에 적는다.
  //
  // 사용자 풀은 VU 이상으로 둔다 — 경합도 3단계와 같은 이유로 **CS-6을 여기
  // 섞지 않는다**(7.3). 같은 예약을 취소하는 둘은 어차피 같은 사용자다.
  //
  // **scripts/seed.sh의 cancel 항목과 반드시 같은 값이어야 한다.**
  cancel: { seats: 200, vus: 500, userPool: 500, purpose: 'CS-4 정합성' },
};
