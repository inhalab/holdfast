...
scenarios/
  ...
  sustained.js         지속 경합 시나리오 (7.2.2). 좌석 1석에 계속 겨루게 해
                       4.5.1을 시험한다 (7.5.1)
  quota.js             **할당량 경합 (CS-6, 7.2.3). 정합성 축 전용이다.**
                       사용자 풀을 VU보다 훨씬 작게 두어 같은 사용자의 요청이
                       서로 다른 좌석을 동시에 노리게 한다 — 좌석 락이 못 막는
                       유일한 축이다(1.1). p95·TPS를 읽지 않는다
  cancel.js            **취소·재선점 경합 (CS-4, 7.2.4). 정합성 축 전용이다.**
                       확정까지 가는 유일한 시나리오다 — 좌석이 SOLD 가 된 뒤
                       돌아오는 구간이 CS-4 다. 취소 둘을 멱등키를 달리해 동시에
                       쏘고, 돌아온 좌석은 다른 VU 가 재선점한다. p95·TPS를 읽지 않는다
...
sql/
  ...
  verify-quota.sql     **Q-1~Q-3 CS-6 검증 (할당량 경합 전용).** 끝 상태가
                       아니라 «구간»을 본다 — 홀드를 곧바로 해제하므로 실행이
                       끝나면 활성 홀드가 0이고, 그것을 세면 새도 0이 된다
  verify-cancel.sql    **C-1~C-4 CS-4 검증 (취소·재선점 전용).** V-1 이 못 보는
                       자리를 본다 — 확정한 좌석을 곧바로 취소하므로 둘 다
                       CANCELLED 가 되면 끝 상태에 흔적이 없다. 좌석별 «확정
                       구간»이 겹쳤는지를 센다
  verify-cancel-selfcheck.sql
                       **검사가 위반을 잡는지 확인한다.** 위반 셋을 한 트랜잭션에
                       심고 세 검사가 집어내는지 본 뒤 ROLLBACK 한다. 0이 «안
                       깨졌다»인지 «못 본다»인지 가르지 못하면 그 0은 판정이 아니다
...
scripts/
  ...
  run-quota.sh         **할당량 경합 실행기 (CS-6).** 전략 하나로 3회. 동시 점유
                       최댓값이 상한에 닿지 못하면 그 회차를 폐기한다 — 그때의
                       «0»은 안 깨진 것이 아니라 밟지 못한 것이다
  deadlock-selfcheck.sh **데드락 검사가 잡는지 확인한다 (7.2.1).** 두 세션이
                       좌석 둘을 반대 순서로 잡아 데드락을 하나 만들고 카운터가
                       오르는지 본다. 둘 다 ROLLBACK 한다 — 0 이 «없었다»인지
                       «못 본다»인지 가르지 못하면 그 0 은 판정이 아니다
  run-cancel.sh        **취소·재선점 실행기 (CS-4).** 전략 하나로 3회. 재선점이
                       0이면 그 회차를 폐기한다 — 그때의 «0»은 안 깨진 것이
                       아니라 밟지 못한 것이다
