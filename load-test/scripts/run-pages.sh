...
#   3. 조건을 **번갈아** 측정 (아래 for 루프의 주석 — 몰아서 돌면 드리프트가
#      조건 효과로 읽힌다)
#        none : 배경 부하 없음
#        low  : 배경에 저경합 부하(1000석 / 100 VU)를 깔고 그 위에서 화면을 잰다
#   4. 조건당 3회, 화면별 p95의 중앙값 (summarize-pages.mjs)
#
# 사용:
...
# **조건을 번갈아 돈다. 조건별로 몰아서 돌지 않는다.**
#
# 처음에는 무부하 3회를 먼저 다 돌고 부하 중 3회를 돌았는데, 그렇게 재자
# **부하 중이 무부하보다 빨랐다** — 검표 단말 0.53배, 프로그램 목록 0.66배.
# 부하가 화면을 빠르게 만들 수는 없으므로 그 값은 성립하지 않는다.
#
# 원인은 시간 드리프트다. 무부하 회차가 5.1 → 4.5 → 4.0처럼 **갈수록 빨라졌고**,
# 워밍업 30초를 지나고도 JIT이 계속 더워지고 있었다. 조건을 시간 순으로 배치하면
# 그 드리프트가 통째로 "조건의 효과"로 읽힌다.
#
# **7.8.4가 정확히 이 함정에서 판정 하나를 회수했다.** 대조군을 세션 앞머리에
# 한 번만 재고 전략을 뒤에 붙였더니, 본 것이 전략의 비용이 아니라 배치였다.
# 번갈아 돌면 드리프트가 두 조건에 고르게 실린다.
